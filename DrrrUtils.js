...
(async function () {
        ...
        if (message.message && message.from) {
          if (message.from.tripcode) {
            if (
              ...
              message.from.name !== globalRoom.profile.name &&
              this.storage.includes(message.from.tripcode)
            ) {
  ...
  */
  ...
  async function banUser(user) {
    return await postAPI('room', ['report_and_ban_user', user.id]);
  }

  async function kickUser(user) {
    return await postAPI('room', ['kick', user.id]);
  }

  async function sendMessage(message) {
    return await postAPI('room', ['message', message]);
  }
  ...
  async function registerEventHandlers() {
    for (const module of modules) {
      ...
      button.onclick = () => {
        ...
        moduleContainer.style.display === 'none' || moduleContainer.style.display === ''
